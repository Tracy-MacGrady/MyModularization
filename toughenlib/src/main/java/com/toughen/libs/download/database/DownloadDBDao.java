...
public class DownloadDBDao {
    ...
    public long delete(SQLiteDatabase db, DownloadDBEntity entity) {
        return db.delete(DownloadDBEntity.class.getSimpleName().toUpperCase(), "DOWNLOADPATH=? AND THREADID=?", new String[]{entity.getDownloadPath(), entity.getThreadID() + ""});
    }

    public long update(SQLiteDatabase db, DownloadDBEntity entity) {
        ContentValues contentValues = new ContentValues();
        contentValues.put("DOWNLOADLENGTH", entity.getDownloadLength());
        return db.update(DownloadDBEntity.class.getSimpleName().toUpperCase(), contentValues, "DOWNLOADPATH=? AND THREADID=?", new String[]{entity.getDownloadPath(), entity.getThreadID() + ""});
    }

    public List<DownloadDBEntity> selectList(SQLiteDatabase db, String downloadPath) {
        List<DownloadDBEntity> list = new ArrayList<>();
        Cursor cursor = db.query(DownloadDBEntity.class.getSimpleName().toUpperCase(), null, "DOWNLOADPATH=?", new String[]{downloadPath}, null, null, null);
